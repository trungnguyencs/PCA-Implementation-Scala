...
object Main extends App {
  ...
  val fname = "4_100_2_2"
  val inp_size = fname.split("_").map(_.trim)

  val data = csvread(new File("../data/" + fname + ".csv"),',').t
  ...
  println(s"--- Memory usage: ${(runtime.totalMemory - runtime.freeMemory) / mb} Mbytes --- \n")
